...
- **Integrated Map API** to render the search results in a commonly used and easy to understand formating, allowing the user to see where the results are on the map. The map is interactive: it can be clicked to generate directions to a map location, pinchable to zoom in/out, and has the standard types of interactivity that integrated map APIs have.
